...
if(isset($_POST['btn_submit']))
	{
		...
		{
			$ins = "insert into tbl_complaint (complaint_content,complainttype_id,user_id,complaint_date) values('".$complaint."','".$complainttype."','".$_SESSION["userid"]."',curdate())";
...
<form id="form1" name="form1" method="post" action="">
  <table >
    <tr>
	  ...
	  {
     ?>
		 <option value="<?php echo $data['complainttype_id'];?>" 
		
        
         ><?php echo $data['complainttype_name']; ?></option >
           <?php
	  }
	?>
		 </td>
  ...
  <p>&nbsp;</p>
      ...
      <tr>
        ...
        <td>reply</td>
         ...
         <?php
	  $i=0;
	  $sel = "select * from tbl_complaint c inner join tbl_complainttype ct on ct.complainttype_id=c.complainttype_id where  user_id ='".$_SESSION["userid"]."'";
	  $row = $conn->query($sel);
	  while($data = $row->fetch_assoc())
	  {
		  $i++;
